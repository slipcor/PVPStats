...
public class UpdateManager {
	 ...
	 */
	public static void updateCheck(Plugin p) {
	    String pluginUrlString = "http://dev.bukkit.org/server-mods/pvp-stats/files.rss";
	    try {
	      ...
	      if (firstNode.getNodeType() == 1) {
	        ...
	        String sThisVersion = instance.getDescription().getVersion();
	...
	public static void message(Player player) {
		if (player == null || !(player instanceof Player)) {
			if (!msg) {
				instance.getServer().getLogger().info("[PVP Stats] You are on latest version!");
			} else {
				if (outdated) {
					instance.getServer().getLogger().warning("[PVP Stats] You are using v"+vThis+", an outdated version! Latest: "+vOnline);
				} else {
					instance.getServer().getLogger().warning("[PVP Stats] You are using v"+vThis+", an experimental version! Latest stable: "+vOnline);
				}
			}
		}
		if (!msg) {
		...
		if (outdated) {
			player.sendMessage("You are using "+colorize("v"+vThis,'o')+", an outdated version! Latest: "+colorize("v"+vOnline,'s'));
		} else {
			player.sendMessage("You are using "+colorize("v"+vThis,'e')+", an experimental version! Latest stable: "+colorize("v"+vOnline,'s'));
		}
	}
